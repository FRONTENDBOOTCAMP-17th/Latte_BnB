...
function buildHeader() {
  ...
  header.className =
    'fixed top-0 left-0 right-0 z-50 min-w-100 bg-shark-100 flex flex-col lg:flex-row items-center lg:justify-between px-[clamp(20px,4vw,80px)] pb-5 pt-2 lg:pt-4 border-b-2 border-shark-200';
...
function buildSearchBar() {
  ...
  div.className =
    'lg:ml-auto max-w-xl w-full relative hover:scale-[1.02] transition-transform duration-200';
  div.innerHTML = `
  <input type="text" id="searchInput" class="w-full h-10 lg:h-15 bg-white text-shark-800 px-4 py-2 rounded-3xl lg:rounded-4xl shadow-md placeholder:text-shark-600 placeholder:font-medium placeholder:text-center placeholder:bg-[url(./src/assets/search.svg)] placeholder:bg-no-repeat placeholder:bg-position-[calc(50%-6rem)_75%] placeholder:bg-size-[16px_16px]" placeholder="특별한 공간 탐색하기" />
  <button id="searchBtn" type="button" class="bg-primary-500 hover:bg-primary-500/85 text-white absolute p-2 right-1 lg:right-2 top-1/2 -translate-y-1/2 rounded-[50%] cursor-pointer">
